...
class _EditDiaryScreenState extends State<EditDiaryScreen> {
  final DiaryUserBloc _diaryUserBloc = DiaryUserBloc();
  ...
  void initState() {
    ...
    _moodBloc = MoodBloc();
    _moodBloc.getMood();
    _diaryUserBloc.getIdDU(widget.id);
  }
  ...
  Widget buildIdDiary(){
    final height = MediaQuery.of(context).size.height;
    final width = MediaQuery.of(context).size.width;
    return BlocBuilder<DiaryUserBloc, DiaryuserState>(
        bloc : _diaryUserBloc..add(GetIdDiary(id: widget.id)),
        builder: (context, state){
          if (state is DiaryUserLoading) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          } else if (state is DiaryUserFailure) {
            return ItemLoadFail(
                msg: state.error,
                onRefresh: () {
                  _diaryUserBloc.add(GetIdDiary(id: widget.id));
                });
          } else if (state is DiaryUserSuccess) {
            return Column(
